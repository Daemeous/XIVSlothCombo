        ...
        internal class SAM_ST_YukikazeCombo : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID == Yukikaze)
                {
                    ...
                    {
                        if (TargetNeedsPositionals() && IsEnabled(CustomComboPreset.SAM_TrueNorth) && GetBuffStacks(Buffs.MeikyoShisui) > 0 && !HasEffect(All.Buffs.TrueNorth) && GetRemainingCharges(All.TrueNorth) > 0 && All.TrueNorth.LevelChecked())
        ...
        internal class SAM_ST_GekkoCombo : CustomCombo
        {
            ...
            protected override uint Invoke(uint actionID, uint lastComboMove, float comboTime, byte level)
            {
                if (actionID == Gekko)
                {
                    ...
                    if (CanSpellWeave(actionID) && TargetNeedsPositionals() && IsEnabled(CustomComboPreset.SAM_TrueNorth) &&  GetBuffStacks(Buffs.MeikyoShisui) > 0 && !HasEffect(All.Buffs.TrueNorth) && GetRemainingCharges(All.TrueNorth) > 0 && All.TrueNorth.LevelChecked())
                        return All.TrueNorth;
